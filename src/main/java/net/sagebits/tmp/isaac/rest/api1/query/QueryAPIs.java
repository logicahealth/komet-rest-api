...
import net.sagebits.tmp.isaac.rest.api1.data.query.RestQueryResultsRow;
...
public class QueryAPIs
{
	 ...
	 * <br>Example:
	 * <pre>
	 * &ltrestQueryResults&gt
     *     &ltrow&gt
     *         &ltresult&gt79a92f9e-cd93-5537-984c-c9aa4532e59d&lt/result&gt
     *     &lt/rows&gt
     *     &ltrows&gt
     *         &ltresult&gtf02874c5-186b-53c4-9054-f819975a9814&lt/result&gt
     *     &lt/row&gt
     * &lt/restQueryResults&gt
	 * </pre>
	...
	public RestQueryResults flworQuery(
			String flworQueryXml) throws RestException {

		SecurityUtils.validateRole(securityContext, getClass());

		// Populate supported query parameters list
		// In this case no query parameters are supported
		final String[] supportedParameters = new String[] {};

		// Each API method should validate that passed query parameters are appropriate for this request
		RequestParameters.validateParameterNamesAgainstSupportedNames(RequestInfo.get().getParameters(), supportedParameters);

		log.trace("Executing FLWOR query: \n%s", flworQueryXml);

		Query queryFromXml = null;
		QueryFromXmlProvider queryParserService = null;
		try {
			try {
				queryParserService = Get.service(QueryFromXmlProvider.class);
			} catch (Exception e) {
				log.trace("Service " + QueryFromXmlProvider.class.getName() + " unavailable from LookupService, which may be the result of a Maven configuration problem.  Using local instance of provider, instead.");
			}
			if (queryParserService == null) {
				queryParserService = new QueryFromXmlProvider();
			}
		...
		List<List<String>> queryResultFromIsaacAsRowsOfColumns = Collections.EMPTY_LIST;
		try {
			queryResultFromIsaacAsRowsOfColumns = queryFromXml.executeQuery();
		} catch (Exception e) {
		...
		return createResponseEntity(queryResultFromIsaacAsRowsOfColumns);
	}

	private static RestQueryResults createResponseEntity(List<List<String>> queryResultFromIsaacAsRowsOfStrings) {
		List<RestQueryResultsRow> resultRows = new LinkedList<>();

		for (List<String> aRowOfStringsFromIsaacQueryResult : queryResultFromIsaacAsRowsOfStrings) {
			RestQueryResultsRow aRowOfStrings = new RestQueryResultsRow(aRowOfStringsFromIsaacQueryResult.toArray(new String[aRowOfStringsFromIsaacQueryResult.size()])); // a row of strings

			resultRows.add(aRowOfStrings);
		}
		RestQueryResults restQueryResultsToReturn = new RestQueryResults(resultRows.toArray(new RestQueryResultsRow[resultRows.size()]));

		log.info("Returning result of FLWOR query: " + restQueryResultsToReturn);
